...
由于数据集过大，上传项目前将数据全部删除，再次测试时清务必先在interview中再次生成数据。
...
这样查询10w数据需要0.09s，100W数据需要1.29s,版本1的代码在release/version1中。
...
对此我进行了测试，如果用多线程加锁希尔排序实现，即便1W数据查询也需要1.51s,10w数据需要的时间甚至达到了200s，根本不能接受。（这里可以看出线程锁和memcpy的开销有多大）,相应代码在release/version3中。
...
1.由于不要文件缓冲还要随机读取文件，程序非常慢，这不是排序的原因，看输出排序仍然只有0.01s，原因在于文件IO非常非常慢，可以运行到3min(但是实际情况下，select并不需要输出这么多的数据到文件中，输出到终端中居多；而且如果文件设置缓冲能大大加快运行速度，内存占用也小于2M，这里不设置文件缓冲是为了实现题目要求)

2.在快排完成后看top命令，内存涨到了1.3M，我认为这不是程序的问题，因为快排后使用的所有内存我确定都会被释放，我推测是top把运行的所有堆栈加到开销中，在堆栈空间被释放时却不减去这部分开销，这很可能是top统计数据的问题；

![a2](./photos/a2.png)

查询10W数据内存小于1M的代码在mem_save_1包中。

## 测试

在release的每个版本中都做了回归测试，除了version3_alter是为了证明希尔排序的改进无法用多线程实现外测试结果均正确；  
...
version5为堆排序，version6为基数排序，version7为归并。算法效率为快排>归并>堆>基数。

归并排序在规模为10W时效率和快排差的不多，规模为100W时效率和快排就差的有点远了，主要是因为堆排序需要一倍的临时内存。请注意该内存最好的方式是定义为全局变量，否则栈空间随着递归将被巨额消耗，甚至达到10GB以上(其实递归的栈不能被及时释放)

堆排序比快排慢一倍则基本符合预期：复杂度大约为2*n*logn

而基数排序比快排慢十倍，这说明把所有指令视为同样时间复杂度是说不同的，归排序理论上应该很快(radix*n),实则因为链表的插入删除操作需要内陷到系统环进行，时间复杂度远高于一般指令。